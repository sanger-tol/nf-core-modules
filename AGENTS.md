# sanger-tol/nf-core-modules: agents

This is the AI agent context file for the sanger-tol modules repository. All AI agents and coding assistants **MUST** read and strictly follow the rules contained in this document.
...
You are working in the sanger-tol modules repository. Repository structure:
...
│   └── sanger-tol             // directory containing sanger-tol modules, described below
├── nf-test.config       // configuration for nf-test
├── subworkflows
│   ├── sanger-tol       // directory containing sanger-tol subworkflows, described below
...
### modules/sanger-tol structure

Module directories are under modules/sanger-tol, either directly or with one nesting level. The following rules apply:

- If a tool has only one command, it is included directly (e.g. modules/sanger-tol/iqtree)
- If a tool has subcommands, they are contained as subdirectories of the tool directory (e.g. modules/sanger-tol/bedops/convert2bed); if the tool without subcommands has a separate functionality, it is included as a subcommand (e.g. modules/sanger-tol/kraken2/kraken2)
- If a tool has multiple levels of subcommands, the subcommands are concatenated (e.g. `aws s3 ls` would be modules/sanger-tol/aws/s3ls)
...
```
subworkflows/sanger-tol/{subworkflow}/
