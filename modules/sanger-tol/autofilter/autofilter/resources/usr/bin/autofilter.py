...
def get_domain_from_taxid(query_taxid, rankedlineage_path):
    ...
    if domain is None:
        sys.stderr.write(
            f"The domain for taxid ({query_taxid}) was not found in the NCBI rankedlineage.dmp file ({rankedlineage_path})\n"
        )
        sys.exit(1)
    return domain
...
def get_scaff_names(assembly_path):
    ...
    """
    scaffs = list()
    fasta_data = read_fasta_in_chunks(assembly_path)
...
def filter_assembly(assembly_path, scaffs_to_exclude, filtered_assembly_path):
    ...
    for header, seq in fasta_data:
        if header not in scaffs_to_exclude:
            ...
            split_seq = split_with_fixed_row_length(seq, 80)
    ...
    export_list_as_line_break_separated_file(out_list, filtered_assembly_path)
...
def main():
    ...
    if args.generate_decontaminated_assembly:
        filter_assembly(assembly_path, scaffs_to_exclude, filtered_assembly_path)

    if len(scaffs_to_exclude) == 0:
        scaffs_to_exclude = ["NO EXCLUDED SEQUENCES"]

    export_list_as_line_break_separated_file(scaffs_to_exclude, args.rejected_seq)
    ...
    export_list_as_line_break_separated_file(out_csv_list, f"{args.out_prefix}_ABNORMAL_CHECK.csv")
