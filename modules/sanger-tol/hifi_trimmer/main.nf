process HIFI_TRIMMER {
   ...
   path(yaml)
   tuple val(meta1), val(barcode)


   output:
   tuple val(meta), path("*.hifi_trimmer.fast{q,a}.gz")      , emit: filtered
   ...
   suffix = args1.contains('-f') ? "fastq.gz"  : "fasta"
   def barcode_sub = barcode ? "sed -i \"s|SAMPLEBARCODE|${barcode}|g\" ${yaml}" : ''
   """
   $barcode_sub
   hifi_trimmer process_blast $args $blast $yaml -t ${task.cpus}
   hifi_trimmer filter_bam $args1 $bam *.bed.gz ${prefix}.hifi_trimmer.${suffix} -t ${task.cpus}
   ...
   echo '{"stub": "data"}' > ${prefix}.summary.json
