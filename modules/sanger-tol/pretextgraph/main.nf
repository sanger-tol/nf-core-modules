process PRETEXT_GRAPH {
    ...
    tuple val("${task.process}"), val('ucsc'), val("447"), topic: versions, emit: versions_ucsc
    tuple val("${task.process}"), val('PretextGraph'), eval('PretextGraph | sed "/Version/!d; s/.*Version //"'), topic: versions, emit: versions_pretextgraph,
